...
- (void)addDebugLayer:(UIColor *)color view:(UIView *)view{
...
- (void)viewDidLoad {
    ...
    self.swipeTableView.frame = CGRectMake(0, 100, self.view.st_width, 600);
    ...
    [self addDebugLayer:[UIColor redColor] view:self.swipeTableView];
    ...
    self.swipeTableView.st_y = self.tableViewHeader.st_bottom - self.swipeTableView.swipeHeaderTopInset;
}
...
- (void)getDataAtIndex:(NSInteger)index {
    ...
    switch (index) {
        case 0:
            numberOfRows = _type == STControllerTypeNormal?18:30;
            break;
        case 1:
            numberOfRows = _type == STControllerTypeNormal?40:28;
            break;
        case 2:
            numberOfRows = _type == STControllerTypeNormal?35:46;
            break;
        case 3:
...
- (void)getAllData {
    if (_type == STControllerTypeNormal) {
        [_dataDic setObject:@(28) forKey:@(0)];
        [_dataDic setObject:@(30) forKey:@(1)];
        [_dataDic setObject:@(45) forKey:@(2)];
        [_dataDic setObject:@(52) forKey:@(3)];
    }else {
        [_dataDic setObject:@(60) forKey:@(0)];
        [_dataDic setObject:@(72) forKey:@(1)];
        [_dataDic setObject:@(88) forKey:@(2)];
        [_dataDic setObject:@(94) forKey:@(3)];
    }
}
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context{
    if ([keyPath isEqualToString:@"contentOffset"]) {
        ...
        //由于动态更新头部 导致偏移变化
        //有头部默认的偏移
        CGFloat defaultHaveHeaderOffsetY = -(self.swipeTableView.swipeHeaderBar.st_height + self.swipeTableView.swipeHeaderView.st_height + self.swipeTableView.swipeHeaderTopInset + 0.1);
        //没有头部默认的偏移
        CGFloat defaultNoHeaderOffsetY = -(self.swipeTableView.swipeHeaderBar.st_height + self.swipeTableView.swipeHeaderTopInset);
...
//            if (scrollUp && offset.y > defaultOffsetY) {
            if (offset.y > defaultNoHeaderOffsetY) {
                CGFloat y = self.tableViewHeader.st_y;
                if (self.tableViewHeader.superview) {
                    [self.tableViewHeader removeFromSuperview];
                }
                //加上头部后的坐标
                self.swipeTableView.st_y = y - self.swipeTableView.swipeHeaderTopInset;
...
//                [self.swipeTableView reloadData];
            ...
            //往下 需要把头部分开
//            if (!scrollUp && offset.y < defaultOffsetY) {
            if (offset.y < defaultHaveHeaderOffsetY) {
                self.swipeTableView.swipeHeaderView = nil;
//                [self.swipeTableView reloadData];
